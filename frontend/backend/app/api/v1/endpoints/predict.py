...
@router.get("/sample-batch", response_model=list[Dict[str, Any]])
async def sample_batch(size: int = 15):
    """
    Return a batch of real vehicles with predictions from the dataset.
    """
    try:
        return inference_service.get_sample_batch(size)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
